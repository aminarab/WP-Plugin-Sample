 ...
 * Description: Creates an interfaces to manage store / business locations on your website. Useful for showing location bas
 * Version: 1.0.0
...
class wp_simple_location
{
    ...
    public function __construct()
    {
        add_action('init', array(
            $this,
            'set_location_trading_hour_days'
        )); // sets the default trading hour days (used by the content type)
        add_action('init', array(
            $this,
            'register_location_content_type'
        )); // register location content type
        add_action('add_meta_boxes', array(
            $this,
            'add_location_meta_boxes'
        )); // add meta boxes
        add_action('save_post_wp_locations', array(
            $this,
            'save_location'
        )); // save location
        ...
        add_filter('the_content', array(
            $this,
            'prepend_location_meta_to_content'
    ...
    public function set_location_trading_hour_days()
    {
        ...
        $this->wp_location_trading_hour_days = apply_filters('wp_location_trading_hours_days', array(
            'monday' => 'Monday',
            'tuesday' => 'Tuesday',
            'wednesday' => 'Wednesday',
            'thursday' => 'Thursday',
    ...
    {
        add_meta_box('wp_location_meta_box', // id
        'AminMetaBox Location Information', // name
        array(
            $this,
            'location_meta_box_display'
        ), // display function
...
<div class="field-container">
        ...
        ?>
			<div class="field">
		<label for="wp_location_phone">Contact Phone</label> <small>main
			contact number</small> <input type="tel" name="wp_location_phone"
			id="wp_location_phone" value="<?php echo $wp_location_phone;?>" />
	</div>
	<div class="field">
		<label for="wp_location_email">Contact Email</label> <small>Email
			contact</small> <input type="email" name="wp_location_email"
			id="wp_location_email" value="<?php echo $wp_location_email;?>" />
	</div>
	<div class="field">
		<label for="wp_location_address">Address</label> <small>Physical
			address of your location</small>
        ...
        if ($post_type == 'wp_locations' && is_singular('wp_locations')) {
            ...
            if (! empty($this->wp_location_trading_hour_days)) {
                ...
                foreach ($this->wp_location_trading_hour_days as $day_key => $day_value) {
                    ...
                    $html .= '<div style="border:1px solid red;"><b>' . $day_key . '</b></div>' . $trading_hours . '</br>';
                }
                $html .= '</p>';
            }
